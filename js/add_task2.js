...
if(checkIfCategoryandPrioritySelectionIsEmpty()) {
    pushTaskToSpecificColumn();
} else showAlertForPriorityEmptynes();
    showAlertForCategoryEmptynes();
}



function checkIfCategoryandPrioritySelectionIsEmpty() {
   if(task['prior']['name'] != undefined && task['category']['name'] != undefined) {
    return true
   }
}

async function pushTaskToSpecificColumn() {
    if(parameterForTaskCreation == 'todo') {
        pushTaskToTodo();
    } else if(parameterForTaskCreation == 'inProgress') {
        pushTaskToInProgress();
    } else if(parameterForTaskCreation == 'awaitingFeedback') {
        pushTaskToAwaitingFeedback();
    } else if(parameterForTaskCreation == 'done') {
        pushTaskToDone();
    } else {
        task['board'] = 0;
        pushTaskToTodo();
...
function showAlertForPriorityEmptynes() {
if(task['prior']['name'] == undefined) {
    document.getElementById('prio-container-alert').classList.remove('d-none');
 }
}

function showAlertForCategoryEmptynes() {
    if(task['category']['name'] == undefined) {
        document.getElementById('category-required-alert').classList.remove('d-none');
    } 
}
...
function pushTaskToInProgress() {
    boardColumns[1].push(task);
}

/** This function adds the current task to todo/boardcolumns[0] */
function pushTaskToAwaitingFeedback() {
    boardColumns[2].push(task);
}

/** This function adds the current task to todo/boardcolumns[0] */
function pushTaskToDone() {
    boardColumns[3].push(task);
}
...
function actionInvite(formData) {
     const input = 'https://sezgin-guendogdu.developerakademie.net/Join-Projekt/send_invite_mail.php';
     const requestInit = {
         method: 'post',
         body: formData
     };
